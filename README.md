...
Judges can evaluate this project in **two fast ways**:

### Option A: Review Sample Reports Directly on GitHub (No Code Execution Needed)
Inspect pre-generated, full-length CEO dossiers and audit traces generated by our 7-agent boardroom engine directly on GitHub:
* 📄 **[Baseline CEO Decision Dossier (Markdown)](outputs/baseline_decision.md)** — Full 5-stage boardroom output, dynamic strategy comparison, rejected alternative rationale, roadmap, and KPIs.
* 🚨 **[Revised CEO Decision Dossier Post-Surprise (Markdown)](outputs/revised_decision.md)** — Mid-event surprise adaptation, changed fact diffing, re-run agent traces, and revised roadmap.
* 📊 **[Full Boardroom JSON Audit Trace (JSON)](outputs/baseline_trace.json)** — Complete step-by-step state trace across all 7 agents.
* 🎯 **[Official 15 Test Cases Dataset (JSON)](data/official_test_cases.json)** — Complete facts for Theme A (FinSwarm TC1-5), Theme B (SaaS Swarm TC1-5), and Theme C (ChipSwarm TC1-5).

---

### Option B: Run Interactive UI Dashboard locally in 3 Commands
```bash
...
```
> Open **`http://localhost:8501`** in your browser to view the interactive executive dashboard, select official test case presets, and run live baseline / surprise adaptation protocols!
